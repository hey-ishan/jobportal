    ...
    public function boot()
    {
        try {
            if ($settings = SiteSetting::findOrFail(1272)) {
                ...
                // Laravel 10 mail configuration update
                $driver = $settings->mail_driver;
                if ($driver == 'mail') {
                    $driver = 'sendmail';
                }
                // For local development on Windows (Laragon), force the log driver to avoid sendmail crashes
                ...
                config([
                    ...
                    'mail.recieve_to.address' => $settings->mail_to_address,
                    'mail.recieve_to.name' => $settings->mail_to_name,
                ]);
            ...
            // Do nothing if DB is unavailable during deployment build
        }
    }
